...
The simulation API consists of a pair of dataframes (books and shelves) that represent the contents of the library and a set of methods that act upon those dataframes. The flowchart below outlines the tasks performed by those methods and the logic for each run. 

![](booksimAPI_logic.png)

Dataframes for books and shelves are first initialized along with several counter variables that represent the number of iterations, number of current full shelves and number of discarded books. A reshelve threshold is also set to 5 for our simulation, along with the amount of initial free shelf space for each set of experiments.  

After initialization, the system enters a loop and each method is called on the books dataframe. After any additive or subtractive method call the "update shelves" method is called to update the status of each shelf. The solid arrow in the chart represents the flow of the books dataframe through this series of changes. 

Once the number of full shelves is greater than or equal to the reshelf threshold for the experiment, the loop breaks and the results are returned to the parent scope. 

In a complete simulation, 100 such experiments are performed and the results aggregated for a sequence of increasing intial free shelf space values as described above in **Running the Simulation**.
