...
\includegraphics[width=300pt]{Table1.png}
...
\includegraphics[width=400pt]{boxes2.png}
...
\includegraphics[width=300pt]{Table2.png}
...
\includegraphics[width=350pt]{offsitebks-plt.png}

--

For these reasons it would be reasonable to conclude, that for this small simulated library collection, it would be adventageous to leave 5 inches of free space on the shelves. This both minimizes shifting activities and leaves the library with little risk of having to send materials in this section off site. If the library does not have access to an off-site option or additional shelving though, 4.5 inches is the maximum amount of space that should be left at the end of a shelf.

# Summary and Future Works

The purpose of our research was to attempt to apply Monte Carlo techniques to the problem of minimizing shelf shifting activities in libraries while maximizing the number of volumes that can be kept on the library's shelves. The resulting model has a high degree of face validity and stands as a foundation for a flexible protocol that could be adapted to suit any library subcollection so long as metrics are available to suit the neccessary assumptions and parameters outlined herein. This simulation could also inform librarians as to what metrics would be needed to take advantage of this tool.

Additionally, the results of this simulation approach could be used to help libraries improve their ability to predict when a shelf shifting activities will be required and allow the library to take steps to minimize the need for shelf shifting, as well as more effectively plan and budget for any required shelf shifts when they do occur. And because this code is now open, this approach is accessible to libraries with very small budgets. 

It is important to note though that the specific conclusions of the herein described simulation apply *only* to a small, simplified subcollection of the Wolbach Library. Future work could inform this simulation's scalability and generalizability so that this method could be applied to larger and more complex library collections. The current simulation represents only 1/60th of Wolbach's physical collection and library collections are typically much larger than 1000 volumes. We were limited by processing power and granularity of collection development metrics and therefore limited the scale of the simulated collection. It would also be useful, through future work, to augment the current model to reflect public library collections where textbooks are not a substantial portion of the collection and to factor in seasonality of circulation metrics. Similarly, one could factor projected decreases in physical aquisitions due to shifts toward ebook purchasing into the model to more realistically reflect the complexities of modeling a physical library collection.
