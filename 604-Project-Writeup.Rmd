...
- __Amount of Initial Free Shelf Space__: The simulation examines a series of initial free shelf space values ranging from __3 inches__ up to __6 inches__ in 0.5 inch increments. (*NOTE: Six inches was selected as the upper bound for the range due to coincide with the "width of two human hands" shelf free space guideline espoused in many anecdotal library management discussions*). The amount of initial free shelf space is used during shelving of the virtual library collection at the start of each round of the simulation, with each of the 50 shelves guaranteed to have no less than the indicated amount of free space. 
...
After initialization, the simulation enters a process loop wherein the methods mentioned previously are applied to the virtual library. Each method applied is representative of a particular library process. Specifically, books are weeded from the collection; new textbooks and copies thereof are added to the shelves; new non-textbooks are added to the shelves; some number of books in circulation are returned to the library (i.e., "checked in") and reshelved; some number of books are checked out by library patrons; and some number of textbooks are selected for "de-duplication", wherein excess copies of outdated textbooks are removed from the collection. After each iteration of this sequence the virtual library shelves are examined to determine whether the "reshelf" threshold has been exceeded. If so, the simulation terminates and a count of the number of successful process flow iterations is returned along with a count of the books that were sent to off site storage prior to the start of the simulation.
...
Of primary concern in the development of any simulation model is determining whether or not the behavior of the fabricated model is sufficiently representative of "real world" behavior to allow for its use as a practical decision-making tool. Objective model verification and validation generally requires the collection of fairly detailed metrics from the actual real world systems being modeled. In our case, while we were able to accummulate a variety of general metrics related to the operations of the Wolbach Library, due to both time and budgetary constraints we were unable to collect sufficient operational metrics to allow us to objectively verify and validate all aspects of the model described herein. Instead, we have employed a subjective model verification and validation approach that relies upon the subject matter expertise of a member of our team who is currently employed by the Wolbach Library (Daina Bouquin). Through her expertise we were able to confirm the reasonableness of all model assumptions as well as the reasonableness of the model's output. Furthermore, her subject matter expertise has allowed us to achieve a high level of "face validity" for the model. As a result, we are confident that the model described herein can potentially be of use to others who are responsible for shelf space management at other libraries. However, the lack of availability of a truly objective model validation via comparison of the model's behavior to "real world" metrics represents a relevant model usage caveat.

# Results

The table below summarizes the results of running the simulation 100 times for each of the previously defined initial free shelf space values. The __Inches__ column represents the initial free shelf space value for the respective sets of 100 simulation runs while the __mu__ column represents the average number of simulated process loops completed prior to at least 5 virtual library shelves become filled with books. The median, minimum, and maximum number of completed simulated process loops are shown in the __mdn__, __min__, and __max__ columns, respectively, while the __mu_delbks__ column represents the average number of books shipped offsite due to lack of shelf space for the respective set of 100 simulation runs.

![Image](table.png)

As we can see in the table, increasing the initial free shelf space value from 3 inches to 3.5 inches does not appear to substantively increase the number of times the simulated process loop can be completed prior to the shelves becoming filled. The same can be said for increasing that value from 4 inches to 4.5 inches. However, increasing the value from 3 inches to 4 inches or from 4 inches to 5 inches does appear to substantively increase the number of times the simulated process loop can be completed prior to the shelves become filled. The boxplots shown below provide further evidence for this observation.

![Image](boxes.png)

In fact, the boxplots also appear to demonstrate a substantive delay to the need for shelf shifting if the initial free shelf space value is increased beyond 5 inches by any 0.5 inch increment. However, as shown in Table 1, this delay in the need for shelf shifting comes at the cost of having to send an ever increasing number of books to offsite storage. 

### DAINA - YOUR INPUT NEEDED HERE - What do we want to conclude regarding the initial free shelf space amount?? Should it be no more than 4.5 inches or do we want to conclude that 5 inches is acceptable, or some other amount?? 
